...
const HTTPComponent = ({ onClose }) => {
    const description = {
        name: "HTTP",
        fullName: "Hypertext Transfer Protocol",
        description: "The foundation of web communication. A stateless request-response protocol for transferring hypertext documents.",

        packetStructure: {
            requestLine: "METHOD URI VERSION - GET, POST, PUT, DELETE, etc.",
            statusLine: "VERSION CODE REASON - e.g., HTTP/1.1 200 OK",
            headers: "Key-Value pairs - Content-Type, Content-Length, Authorization, etc.",
            body: "Optional message body containing the actual data/content"
        },

        compareWith: {
            HTTPS: "HTTPS is HTTP wrapped with TLS encryption for security",
            HTTP2: "HTTP/2 improves performance with multiplexing and compression",
            HTTP3: "HTTP/3 uses QUIC protocol instead of TCP for faster connection setup",
            WebSocket: "WebSocket maintains persistent connection; HTTP is request-response based"
        }
    };
