...
const WebSocketComponent = ({ onClose }) => {
    const description = {
        name: "WebSocket",
        fullName: "WebSocket Protocol",
        description: "A protocol providing full-duplex communication over a single TCP connection, enabling persistent bidirectional data exchange.",

        packetStructure: {
            openingHandshake: "HTTP upgrade request transitioning to WebSocket",
            frame: "Opcode, payload length, masking, actual data",
            controlFrames: "Close, Ping, Pong frames for connection management",
            dataFrames: "Text or binary frames carrying application data",
            subprotocal: "Optional protocol specification for specialized use"
        },

        compareWith: {
            HTTP: "HTTP is request-response; WebSocket is persistent bidirectional",
            "Server-Sent Events": "SSE is unidirectional from server; WebSocket is bidirectional",
            polling: "Polling repeatedly queries; WebSocket maintains persistent connection",
            gRPC: "gRPC is for service-to-service; WebSocket for client-server real-time"
        }
    };
